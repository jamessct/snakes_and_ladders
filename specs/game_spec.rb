...
class TestGame < Minitest::Test
  ...
  def test_can_play_current_player
  ...
  def test_can_move_current_player
    ...
    assert_equal({name:"Jay", position:2}, @game.current_player() )
  end

  def test_taking_turn_moves_current_player
    @game.play_turn(5)
    assert_equal({name:"Jay", position:5}, @game.players[0])
  end
  ...
  def test_has_target
    assert_equal(8, @game.target)
  end

  def test_can_show_winner
    @game.move_current_player(8)
    assert_equal( {name:"Jay", position: 8}, @game.winner() )
  end

  def test_show_no_winner
